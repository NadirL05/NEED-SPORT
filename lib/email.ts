...
const FROM = process.env.EMAIL_FROM ?? 'MAILLO. <commandes@maillo.fr>'
...
export async function sendOrderConfirmationToCustomer(data: OrderEmailData) {
  ...
  const html = wrapper(`
    ...
    <p style="color:#6B7280;font-size:15px;margin:0 0 28px;line-height:1.5">
      ...
      Ta commande a bien été reçue et est en cours de préparation. Tu recevras une notification dès qu'elle est expédiée.
    </p>
    ...
    <p style="color:#9CA3AF;font-size:13px;margin:6px 0 0">Réf. commande : <code style="font-family:monospace;color:#6B7280">${data.orderId}</code></p>
  `)
...
export async function sendNewOrderToAdmin(data: OrderEmailData) {
  ...
  const html = wrapper(`
    <div style="background:#FEF3C7;border:1px solid #FDE68A;border-radius:8px;padding:14px 16px;margin-bottom:24px">
      <span style="font-size:13px;font-weight:600;color:#92400E">🛒 Nouvelle commande à préparer</span>
    </div>

    <p style="font-size:14px;color:#374151;margin:0 0 4px"><strong>Client :</strong> ${data.customerName ?? '—'} (${data.customerEmail ?? '—'})</p>
    <p style="font-size:14px;color:#374151;margin:0 0 20px"><strong>Réf :</strong> <code style="font-family:monospace">${data.orderId}</code></p>
    ...
    <table width="100%" cellpadding="0" cellspacing="0">
      ${itemsTable(data.items)}
      <tr>
        <td colspan="2" style="padding:12px 0 0;font-weight:700;color:#111827;font-size:14px">Total encaissé</td>
...
export async function sendShipmentRequestToTransporter(data: OrderEmailData) {
  ...
  const html = wrapper(`
    <div style="background:#D1FAE5;border:1px solid #A7F3D0;border-radius:8px;padding:14px 16px;margin-bottom:24px">
      <span style="font-size:13px;font-weight:600;color:#065F46">📦 Nouveau colis à enlever et livrer</span>
    </div>

    <p style="font-size:14px;color:#374151;margin:0 0 4px"><strong>Destinataire :</strong> ${data.customerName ?? '—'}</p>
    <p style="font-size:14px;color:#374151;margin:0 0 20px"><strong>Réf. commande :</strong> <code style="font-family:monospace">${data.orderId}</code></p>
    ...
    <p style="color:#6B7280;font-size:13px;margin:24px 0 0">Le colis sera disponible à l'enlèvement dès confirmation de l'expédition dans le portail fournisseur.</p>
  `)

  await resend.emails.send({
    from: FROM,
    to:   TRANSPORTER_EMAIL,
    subject: `Enlèvement + livraison — ${data.customerName ?? data.orderId.slice(0, 12)} (${formatPrice(data.totalEur)})`,
    html,
  })
}
