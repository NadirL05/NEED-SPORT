...
function EmailGate({ orderId, onVerified }: EmailGateProps) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 401) {
          setError('Email non reconnu pour cette commande.')
        ...
        <div className="w-full max-w-lg bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <div className="px-6 pt-8 pb-6 sm:px-8 sm:pt-10">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h1 className="text-lg font-bold text-gray-900 leading-tight">Vérification</h1>
                <p className="text-sm text-gray-400 font-mono">{shortRef}</p>
              </div>
            </div>
          ...
          <form onSubmit={handleSubmit} className="px-6 py-6 sm:px-8 sm:py-8 space-y-4">
            ...
            {error && (
              <div className="flex items-start gap-2.5 rounded-xl bg-red-50 border border-red-100 px-4 py-3" role="alert">
                <svg className="h-4 w-4 text-red-500 mt-0.5 shrink-0" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                  <path
                    fillRule="evenodd"
        ...
        <div className="w-full max-w-lg space-y-4">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 px-6 py-5 sm:px-8 sm:py-6 flex items-center justify-between gap-4">
            ...
            <p className="text-xs text-gray-400 text-right leading-relaxed">
              Commandé le<br />
              <span className="text-gray-600 font-medium">{formatDate(order.createdAt)}</span>
            </p>
          </div>
